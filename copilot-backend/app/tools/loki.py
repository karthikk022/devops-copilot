...
class LokiClient:
    ...
    async def _ensure(self) -> httpx.AsyncClient:
        if self._client is None:
            self._client = httpx.AsyncClient(
                base_url=self.base_url, timeout=httpx.Timeout(20.0, connect=5.0)
            )
        return self._client
    ...
    async def query_range(
        self, query: str, start: int, end: int, limit: int = 200
    ) -> dict:
...
class LokiQuery(Tool):
    ...
    async def execute(self, query: str, minutes: int = 15, limit: int = 100) -> str:
        ...
        try:
            ...
            return json.dumps(
                {"error": f"Loki returned {e.response.status_code}", "body": body}
            )
        ...
        if not lines:
            return json.dumps(
                {"query": query, "range_minutes": minutes, "result": "no matching logs"}
            )
